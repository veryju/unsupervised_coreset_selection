...
#def select(model: nn.Module, dataset: Dataset, enable_intersect: bool, intersect_method: str, 
#           half_core_indices : np.array, sim_loss : np.array, mixing_rate : float,
#           current: np.array, pool: np.array, budget: int, method: str, batch_size: int,
...
def select(model: nn.Module, dataset: Dataset, current: np.array, 
    ...
    if method == 'random':
        ...
        if method == 'least_confidence':
            ...
            #if enable_intersect:
                #if intersect_method == 'union':
                #    core_loss = np.array([sim_loss[i] for i in pool if i not in current_set])
                #    sumvector = mixing_rate*core_loss - (1-mixing_rate)*probs 
                #    indices = sumvector.argsort(axis=0)[::-1]
    ...
    #if enable_intersect:
    #    if intersect_method == 'naive': # choose intersection between active and coreset
    #        new=np.array(list(set(new).intersection(set(half_core_indices))))
    #    elif intersect_method == 'union':
    #        print('##Union already complished')
    #    else:
    #        raise NotImplementedError(f"'{intersect_method}' method doesn't exist")
