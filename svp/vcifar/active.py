...
from svp.vcifar.datasets import create_dataset
from svp.vcifar.train import create_model_and_optimizer
...
def active(run_dir: str = './run',
    ...
    if precomputed_selection is not None:
        ...
        if coreset_path is not None:
            ...
            print('####################### core-set path :#######',coreset_path)                      # size : 50,000
            if isCoreLoss:
                ...
                core_loss = np.zeros(50000) # subset core loss in CIFAR-10 indices order
                sim_wloss = np.zeros(50000)
                norm_sim_wloss = np.zeros(50000) # extract cossim in CIFAR indices
            ...
            if initial_balance:
                if isCoreLoss:
                    labeled = utils.split_balanced_core_indices(train_dataset, core_indices, norm_sim_loss, isCoreset,
                                                              initial_num_per_class, num_classes)
                else:
                    raise NotImplementedError(' core loss path is not specified.')
            else:
                if isCoreset: # core_subset
                    #labeled = core_indices[24000:25000]
                    print("######### Subset bias is applied")
                    #eighty_point = int(50000*0.8)
                    #start_point = int(initial_subset*0.8)
                    #end_point = int(initial_subset*0.2)
                    labeled = np.argsort(norm_sim_wloss)[subset_bias:subset_bias+initial_subset]
                    
                    #labeled = np.argsort(norm_sim_wloss)[eighty_point-start_point:eighty_point+end_point]
            ...
            print(len(set(labeled)),len(labeled),num_classes)
