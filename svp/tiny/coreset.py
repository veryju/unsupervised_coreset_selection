...
from svp.common.coreset import validate_splits
...
def coreset(run_dir: str = './run',
            ...
            subset: int = 10_000, selection_method: str = 'least_confidence',
    ...
    """
    Perform core-set selection on CIFAR10 and CIFAR100.
    ...
    # Binary variable whether or not skipping proxy model
    skip_proxy = False
    ...
    if precomputed_selection is None:
        ...
        if selection_method == 'ours':
            core_indices = np.loadtxt('/home/siit/Jeongwoo/core-set/svp/initial_core_subset_25000.index',dtype=int)
            target_train_indices = core_indices
            skip_proxy = True
        
        if selection_method == 'ours_weight':
            core_indices = np.loadtxt('/home/siit/Jeongwoo/core-set/svp/initial_core_subset_25000.index',dtype=int)
            core_metrics = np.loadtxt('/home/siit/Jeongwoo/core-set/svp/initial_core_subset_25000.metric',dtype=float)
            target_train_indices = core_indices
            skip_proxy = True
    ...
    if train_target:
        ...
        ###### Check data imbalnce #############
        #labels=[]
        #wrapped_loader = tqdm(target_train_loader)
        #for batch_index, (indices, inputs, targets) in enumerate(wrapped_loader):
        #    labels.extend(targets.tolist())
        
        #for i in range(10):
        #    print('Check data imbalance: ',labels.count(i),len(labels))
